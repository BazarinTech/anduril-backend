                    ...
                    <div class="grid grid-cols-1 gap-4">
                        ...
                        <h2 class="mb-4 text-base font-semibold capitalize text-slate-800 dark:text-slate-100">Wallet Records</h2>
                            ...
                            data_table([
                                'id'       => 'wallets',
                                'label'    => 'wallet',
                                'rows'     => $wallets_records,
                                'key'      => 'id',
                                'update'   => 'actions/update_wallets.php',
                                'can_edit' => $isEdit && $isFinance,
                                'search'   => ['email'],
                                'empty'    => 'No wallets yet.',
                                'columns'  => [
                                    ['label' => '#',     'field' => 'id'],
                                    ['label' => 'Email', 'field' => 'email', 'wide' => true,
                                     'hint'  => "The wallet owner's address, joined in from users."],
                                    /*
                                     * The display values carry a "Kes " prefix and thousands
                                     * separators; the editors read the raw columns. The old
                                     * inline editor posted "Kes 1,200.00" into a DECIMAL
                                     * column, which strict MySQL rejects outright.
                                     */
                                    ['label' => 'Balance', 'field' => 'balance', 'value' => 'balance_raw', 'edit' => 'balance',
                                     'type'  => 'number', 'numeric' => true,
                                     'hint'  => 'Adjusting a balance by hand leaves no transaction behind it. The ledger will not explain the change.'],
                                    ['label' => 'Product Income', 'field' => 'income', 'value' => 'income_raw', 'edit' => 'income',
                                     'type'  => 'number', 'numeric' => true],
                                    ['label' => 'Downline Income', 'field' => 'downline', 'value' => 'downline_raw', 'edit' => 'invite_income',
                                     'type'  => 'number', 'numeric' => true],
                                    ['label' => 'Withdrawals', 'field' => 'withdrawals', 'numeric' => true,
                                     'hint'  => 'Sum of successful withdrawals. Derived from the transaction history, not stored.'],
                                    ['label' => 'Deposits', 'field' => 'deposits', 'numeric' => true,
                                     'hint'  => 'Sum of successful deposits. Derived from the transaction history, not stored.'],
                                ],
                            ]);
                            ?>
                        </div>
